...
3. **Rank**: 对 filterView 计算 finalRank = event.score + sum(RANK signals, capped +30)。按 finalRank 降序排列。
4. **Annotate**: 将 ANNOTATION-phase signals 附加到对应 Candidate 的 `contextHints[]`。
5. **Truncate**: 取 top maxSize（默认 40），返回 `BuildResult`。
...
- ContextHints 以何种格式注入 LLM Curation prompt？决定权在 CurateEvents Task 实现时，不在 CandidateBuilder 层。
- Ranked View 中 score + boost 混合排序是否会导致低质量但高 boost 的候选胜出？监控——若出现，Phase 2 可引入 boost 天花板或 boost/score 比例限制。
