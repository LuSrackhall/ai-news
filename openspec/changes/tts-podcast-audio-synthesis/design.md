...
### D2: 默认 edge-tts
免费，pip install。runner 传 M/F 标识给 provider，provider 映射到默认音色。edge-tts: M→YunxiNeural, F→XiaoxiaoNeural。OpenAI: M→onyx, F→nova。MiniMax: M→male-cn, F→female-cn。可通过 TTS_MALE_VOICE/TTS_FEMALE_VOICE 覆盖。
