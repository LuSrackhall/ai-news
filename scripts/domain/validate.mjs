/**
 * Validate Domain — Schema + 内容质量校验
 * 从 validate-output.mjs 迁移，适配 v4 Artifact 结构
 * 支持 v4 Artifact (content.rendered) 和 v3 格式双重输入
 */

// ── Schema 定义（与 v3 一致）──

const ARTICLE_SCHEMA = {
  required: ['hook', 'summary_items'],
  optional: ['deep_items', 'important_items', 'brief_items', 'editorial'],
  fields: {
    hook: { type: 'string', nonEmpty: true },
    summary_items: { type: 'array', minItems: 1, maxItems: 8, itemFields: ['title', 'one_liner'] },
    editorial: { type: 'object', requiredFields: ['observation', 'evidence', 'judgment', 'prediction'] },
    deep_items: { type: 'array', maxItems: 3, itemFields: ['title', 'what_happened', 'details', 'why_matters', 'implications', 'sources'] },
    important_items: { type: 'array', maxItems: 6, itemFields: ['title', 'key_point', 'analysis', 'source'] },
    brief_items: { type: 'array', maxItems: 8, itemFields: ['title', 'fact'] },
  },
}

const SCRIPT_SCHEMA = {
  required: ['hook', 'overview', 'closing'],
  optional: ['deep_items', 'quick_items'],
  fields: {
    hook: { type: 'object', requiredFields: ['text', 'duration_s'] },
    overview: { type: 'object', requiredFields: ['text', 'duration_s'] },
    closing: { type: 'object', requiredFields: ['text', 'duration_s'] },
    deep_items: { type: 'array', maxItems: 3 },
    quick_items: { type: 'array', maxItems: 8 },
  },
}

const EMPTY_EXPRESSIONS = [
  '值得关注', '意义深远', '引发热议', '广泛关注', '意义重大',
  '不容忽视', '令人期待', '前景广阔', '不可忽视',
]

export function createValidateDomain(ctx) {
  // ── Schema 校验（从 v3 迁移）──

  function validateSchema(data, schema) {
    const errors = []
    for (const field of schema.required) {
      if (data[field] === undefined || data[field] === null) errors.push(`${field}: required field missing`)
    }
    for (const [field, rules] of Object.entries(schema.fields)) {
      const value = data[field]
      if (value === undefined || value === null) continue
      if (rules.type === 'string' && rules.nonEmpty && !value) errors.push(`${field}: empty string`)
      if (rules.type === 'array') {
        if (!Array.isArray(value)) { errors.push(`${field}: expected array`); continue }
        if (rules.minItems && value.length < rules.minItems) errors.push(`${field}: need >= ${rules.minItems}`)
        if (rules.maxItems && value.length > rules.maxItems) errors.push(`${field}: max ${rules.maxItems}`)
        if (rules.itemFields) {
          for (let i = 0; i < value.length; i++) {
            for (const f of rules.itemFields) {
              if (value[i][f] === undefined) errors.push(`${field}[${i}].${f}: missing`)
            }
          }
        }
      }
      if (rules.type === 'object' && rules.requiredFields) {
        for (const f of rules.requiredFields) {
          if (value[f] === undefined || value[f] === null || value[f] === '') errors.push(`${field}.${f}: missing or empty`)
        }
      }
    }
    return { passed: errors.length === 0, errors }
  }

  // ── 内容质量校验（从 v3 迁移）──

  function validateContent(articleMarkdown, scriptMarkdown, curatedEvents, articleContent, scriptContent) {
    const warnings = []

    // 1. URL 交叉比对
    const validUrls = new Set((curatedEvents || []).map((e) => e.url).filter(Boolean))
    const articleUrls = (articleMarkdown || '').match(/https?:\/\/[^\s\)]+/g) || []
    const hallucinatedUrls = articleUrls.filter((u) => !validUrls.has(u))
    if (hallucinatedUrls.length > 0) {
      warnings.push({ check: 'hallucinated_urls', detail: `发现 ${hallucinatedUrls.length} 个编造 URL`, urls: hallucinatedUrls })
    }

    // 2. 空洞表述检测
    let emptyCount = 0
    const text = `${articleMarkdown || ''} ${scriptMarkdown || ''}`
    for (const expr of EMPTY_EXPRESSIONS) {
      const matches = text.match(new RegExp(expr, 'g'))
      if (matches) emptyCount += matches.length
    }
    if (emptyCount > 3) {
      warnings.push({ check: 'empty_expressions', detail: `空洞表述 ${emptyCount} 处（阈值 3）` })
    }

    // 3. 口播稿时长校验
    if (scriptContent) {
      const getDur = (s) => s?.durationS || s?.duration_s || 0
      const totalDuration = [
        getDur(scriptContent.hook),
        getDur(scriptContent.overview),
        ...(scriptContent.deepItems || scriptContent.deep_items || []).map(getDur),
        ...(scriptContent.quickItems || scriptContent.quick_items || []).map(getDur),
        getDur(scriptContent.closing),
      ].reduce((a, b) => a + b, 0)
      if (totalDuration < 180 || totalDuration > 300) {
        warnings.push({ check: 'script_duration', detail: `总时长 ${totalDuration}s（期望 180-300）` })
      }
    }

    // 4. editorial 字段长度
    const editorial = articleContent?.editorial
    if (editorial) {
      for (const field of ['observation', 'evidence', 'judgment', 'prediction']) {
        if (editorial[field] && editorial[field].length < 30) {
          warnings.push({ check: 'editorial_too_short', detail: `editorial.${field}: ${editorial[field].length} 字（最少 30）` })
        }
      }
    }

    // 5. deep_items details 含数字
    const deepItems = articleContent?.deepItems || articleContent?.deep_items || []
    for (const item of deepItems) {
      if (item.details && !/\d/.test(item.details)) {
        warnings.push({ check: 'deep_item_no_number', detail: `"${item.title}" 的 details 不含数字` })
      }
    }

    // 6. 事实锚定检查
    const knownNumbers = new Set()
    for (const event of curatedEvents || []) {
      const eventText = `${event.summary || ''} ${event.title || ''}`
      const nums = eventText.match(/\d+[\d.,]*\s*[万亿bBmMtkg%]?/g) || []
      nums.forEach(n => knownNumbers.add(n.replace(/\s/g, '')))
    }

    const articleNumbers = []
    function extractNumbers(obj) {
      if (typeof obj === 'string') {
        const nums = obj.match(/\d+[\d.,]*\s*[万亿bBmMtkg%]?/g) || []
        articleNumbers.push(...nums.map(n => n.replace(/\s/g, '')))
      } else if (Array.isArray(obj)) obj.forEach(extractNumbers)
      else if (typeof obj === 'object' && obj !== null) Object.values(obj).forEach(extractNumbers)
    }
    extractNumbers(articleContent)

    const suspiciousNumbers = articleNumbers.filter(n => {
      if (n.length < 2) return false
      const numPart = n.match(/\d+[\d.,]*/)
      if (!numPart) return false
      const knownNums = [...knownNumbers].map(k => k.match(/\d+[\d.,]*/)?.[0]).filter(Boolean)
      return !knownNums.some(k => k === numPart[0])
    })
    if (suspiciousNumbers.length > 0) {
      warnings.push({ check: 'fact_anchoring', detail: `文章中出现 ${suspiciousNumbers.length} 个原始数据中不存在的数字`, numbers: suspiciousNumbers.slice(0, 5) })
    }

    // 7. 日期一致性检查
    const dateViolations = []
    for (const event of curatedEvents || []) {
      const pubDate = event.sources?.[0]?.publishedAt || event.publishedAt
      if (pubDate) {
        const articleDate = (articleMarkdown || '').match(/\d{4}-\d{2}-\d{2}/)?.[0]
        if (articleDate) {
          const diffHours = Math.abs(new Date(pubDate).getTime() - new Date(articleDate + 'T12:00:00Z').getTime()) / (1000 * 60 * 60)
          if (diffHours > 36) dateViolations.push({ title: event.title?.slice(0, 40), published_at: pubDate })
        }
      }
    }
    if (dateViolations.length > 0) {
      warnings.push({ check: 'date_consistency', detail: `${dateViolations.length} 条超出目标日期 ±36h 窗口` })
    }

    // 8. 日期可靠性警告
    const KNOWN_UNRELIABLE = ['The Verge']
    for (const event of curatedEvents || []) {
      const sourceName = event.sources?.[0]?.name
      if (KNOWN_UNRELIABLE.includes(sourceName)) {
        warnings.push({ check: 'date_reliability_warning', detail: `${sourceName} 的 RSS pubDate 可能不可靠` })
      }
    }

    const critical = warnings.filter((w) =>
      ['hallucinated_urls', 'empty_expressions', 'editorial_too_short', 'date_consistency'].includes(w.check)
    )
    return { passed: critical.length === 0, warnings, summary: { hallucinated_url_count: hallucinatedUrls.length, empty_expression_count: emptyCount } }
  }

  // ── 一致性校验（从 v3 迁移）──

  function validateConsistency(articleContent, scriptContent) {
    const articleTitles = new Set()
    for (const item of articleContent?.deepItems || articleContent?.deep_items || []) articleTitles.add(item.title)
    for (const item of articleContent?.importantItems || articleContent?.important_items || []) articleTitles.add(item.title)

    const scriptTitles = new Set()
    for (const item of scriptContent?.deepItems || scriptContent?.deep_items || []) scriptTitles.add(item.title)
    for (const item of scriptContent?.quickItems || scriptContent?.quick_items || []) scriptTitles.add(item.title)

    const intersection = [...articleTitles].filter(t => scriptTitles.has(t))
    const union = new Set([...articleTitles, ...scriptTitles])
    const consistency = union.size > 0 ? intersection.length / union.size : 1
    const warnings = []
    if (consistency < 0.5) {
      warnings.push({ check: 'article_script_consistency', detail: `一致性 ${(consistency * 100).toFixed(0)}%（阈值 50%）` })
    }
    return { consistency, warnings }
  }

  // ── 核心接口 ──

  return {
    /**
     * @param {Object} articleArtifact - ArticleArtifact（含 content/rendered/meta）
     * @param {Object} scriptArtifact - ScriptArtifact
     * @param {Array} curatedEvents - 当日 Event[]
     */
    run(articleArtifact, scriptArtifact, curatedEvents) {
      const articleContent = articleArtifact?.content || articleArtifact
      const scriptContent = scriptArtifact?.content || scriptArtifact

      const articleMarkdown = articleArtifact?.rendered?.markdown || ''
      const scriptMarkdown = scriptArtifact?.rendered?.markdown || ''

      // Schema 校验（对 v3 兼容格式用 snake_case schema，v4 用 camelCase）
      const articleForSchema = normalizeToV3Keys(articleContent)
      const scriptForSchema = normalizeToV3Keys(scriptContent)
      const articleSchema = validateSchema(articleForSchema, ARTICLE_SCHEMA)
      const scriptSchema = validateSchema(scriptForSchema, SCRIPT_SCHEMA)

      const content = validateContent(articleMarkdown, scriptMarkdown, curatedEvents, articleContent, scriptContent)
      const consistency = validateConsistency(articleContent, scriptContent)

      return {
        articlePassed: articleSchema.passed,
        scriptPassed: scriptSchema.passed,
        contentPassed: content.passed,
        consistency: consistency.consistency,
        details: {
          article_schema: articleSchema,
          script_schema: scriptSchema,
          content: content,
          consistency: consistency,
        },
        validation_passed: articleSchema.passed && scriptSchema.passed && content.passed && consistency.warnings.length === 0,
      }
    },
  }
}

/**
 * 将 v4 camelCase 字段名规范化为 v3 snake_case，供 schema 校验使用
 */
function normalizeToV3Keys(obj) {
  if (!obj || typeof obj !== 'object') return obj
  const map = {
    summaryItems: 'summary_items', oneLiner: 'one_liner',
    deepItems: 'deep_items', whatHappened: 'what_happened',
    whyMatters: 'why_matters', importantItems: 'important_items',
    keyPoint: 'key_point', briefItems: 'brief_items',
    quickItems: 'quick_items', durationS: 'duration_s',
  }
  const result = {}
  for (const [k, v] of Object.entries(obj)) {
    const mapped = map[k] || k
    if (Array.isArray(v)) {
      result[mapped] = v.map(item => typeof item === 'object' ? normalizeToV3Keys(item) : item)
    } else if (typeof v === 'object' && v !== null) {
      result[mapped] = normalizeToV3Keys(v)
    } else {
      result[mapped] = v
    }
  }
  return result
}
