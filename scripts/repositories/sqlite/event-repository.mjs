...
export function createSqliteEventRepository(db) {
  ...
  function toRow(event) {
    return {
      ...
      source_name: event.source?.name || event.source_name || null,
      source_tier: event.source?.tier ?? event.source_tier ?? null,
      source_url: event.source?.url || event.source_url || null,
