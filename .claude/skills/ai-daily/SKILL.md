...
Editorial（Agent 驱动，/daily）
  Agent 读取 SQLite → 选题 → 写文章 → 写口播稿 → 调代码渲染/校验/归档
  输出 → output/<date>/
```
...
### Step 4: 生成口播稿（Agent 执行）

按 `references/EDITORIAL.md` 的口播稿结构，基于文章内容生成口播稿。

**要求：**
- 输出 JSON 结构：hook / overview / deep_items / quick_items / closing
- 总时长 180-300 秒
- 口语化，短句为主

将口播稿 JSON 写入 `output/<date>/script.json`。
